...
**不同种子下的搜索表现。** 在 8² 和 8³ 上，每个种子使用 12 次提案，比较搜索策略及历史信息对照。
当前最优延迟与参考延迟的比值越低越好。三个种子的结果表明，真实历史并未稳定优于无历史或打乱历史。

<p align="center">
  <a href="assets/paper/figure4_policy.png"><img src="assets/paper/figure4_policy.png" alt="8² 和 8³ 在种子 41–43 下的搜索轨迹，对比搜索策略及历史信息；延迟比值越低越好" width="880"></a>
</p>

*论文 Fig. 4。搜索轨迹展示不同策略、历史信息和种子下的差异。*

**各项优化分别带来多少收益。** 8² 上的配对比较分别考察阶段分组、尾部算子融合、核数扩展和行打包。
阶段分组的中位加速比为 **1.92×**，而从四核增加到八核仅为 **1.01×**，表明继续增加并行度的收益趋于有限。

<p align="center">
  <a href="assets/paper/figure5_boundaries.png"><img src="assets/paper/figure5_boundaries.png" alt="8² 上各项优化的配对加速比分布：阶段分组中位收益为 1.92 倍，四核到八核为 1.01 倍" width="880"></a>
</p>

*论文 Fig. 5。加速比大于一表示改善；阶段分组在这些配对变换中带来最大的中位收益。*
...
上述结果均为 Ascend 310P1 上的 FP32 测量。八核延迟对比与 Fig. 3 来自不同计时实验。
Fig. 3 将各优化层级归一化到单核 Staged 基线；Fig. 5 用配对比较考察单项变换；
Fig. 4 展示当前最优延迟与参考延迟的比值。
