...
__global__ void multiply(cufftComplex*z,const cufftComplex*h){int k=blockIdx.x*blockDim.x+threadIdx.x;if(k<512){auto a=z[k],b=h[k];z[k]={a.x*b.x-a.y*b.y,a.x*b.y+a.y*b.x};}}
__global__ void normalize(cufftComplex*y){int k=blockIdx.x*blockDim.x+threadIdx.x;if(k<512){y[k].x/=512;y[k].y/=512;}}
template<int K> __global__ void empty_kernel(cufftComplex*x,cufftComplex*h,cufftComplex*y){}
int main(int argc,char**argv){
 ...
 size_t bytes=512*sizeof(cufftComplex);std::vector<cufftComplex> a(512),b(512);
 ...
 cufftComplex *x,*h,*z,*y;ck(cudaMalloc(&x,bytes));ck(cudaMalloc(&h,bytes));ck(cudaMalloc(&z,bytes));ck(cudaMalloc(&y,bytes));
 ...
 cufftHandle plan;cf(cufftPlan3d(&plan,8,8,8,CUFFT_C2C));cf(cufftSetStream(plan,stream));
 auto pipeline=[&](){
  if(empty){
  ...
  }else{cf(cufftExecC2C(plan,x,z,CUFFT_FORWARD));multiply<<<2,256,0,stream>>>(z,h);ck(cudaGetLastError());cf(cufftExecC2C(plan,z,y,CUFFT_INVERSE));normalize<<<2,256,0,stream>>>(y);ck(cudaGetLastError());}
 };
 ...
 printf("{\"timing_method\":\"host_batch_submit_sync\",\"unit\":\"us_per_pipeline\",\"precision\":\"fp32\",\"warmup\":1000,\"groups\":11,\"iterations\":%d,\"mode\":\"%s\",\"graph_pipelines_per_launch\":%d,\"graph_warmup_pipelines\":%d,\"device_index\":%d,",reps,mode.c_str(),graphSize,graphSize,device);emit("samples_us",total);printf(",");emit("submit_us",submit);printf(",");emit("drain_us",drain);printf("}\n");
