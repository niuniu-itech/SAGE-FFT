// Native vendor FFTWithSize weighted pipeline, with compile-once ACL execution.
...
public:
    ...
    aclopAttr* attributes(bool fft, bool inverse = false) {
        ...
        if (fft) {
            ...
            check(aclopSetAttrString(a, "norm", inverse ? "forward" : "backward"), "norm");
...
int main(int argc, char** argv) {
    if (argc != 7 && argc != 8) {
        std::cerr << "usage: native_fft_pipeline INPUTDIR OUTPUTPREFIX N ITERS WARMUPS GROUPS [--scale-vector]\n";
        std::cerr << "Uses device 0; optional scale-vector uses N^3 normalization values instead of scalar broadcasting.\n";
        return 1;
    }
    ...
    try {
        ...
        vector_scale = argc == 8;
        if (vector_scale && std::string(argv[7]) != "--scale-vector") throw std::runtime_error("unknown optional argument");
        ...
        std::vector<Complex32> scale(vector_scale ? count : 1, {1.0f / static_cast<float>(count), 0.0f});
        Pipeline p;
        p.init();
        ...
        p.tensor(vector_scale ? shape : std::vector<int64_t>{1}, &scale); // 2 normalization
        p.tensor(shape);        // 3 forward spectrum
        p.tensor(shape);        // 4 weighted spectrum
        p.tensor(shape);        // 5 unnormalized inverse
        p.tensor(shape);        // 6 normalized output
        p.operation("FFTWithSize", {0}, 3, p.attributes(true, false));
        p.operation("Mul", {3, 1}, 4, p.attributes(false));
        p.operation("FFTWithSize", {4}, 5, p.attributes(true, true));
        p.operation("Mul", {5, 2}, 6, p.attributes(false));
        ...
        p.run();
        p.synchronize();
        auto forward = p.download(3), before = p.download(6);
    ...
    json << std::setprecision(17)
         << "{\"implementation\":\"native ACL FFTWithSize weighted pipeline\",\"soc\":" << quote(soc)
         << ",\"shape\":[" << n << ',' << n << ',' << n << "],\"batch\":1,\"precision\":\"complex64\""
         << ",\"warmup\":" << warmups << ",\"groups\":" << groups << ",\"iterations\":" << iters
         << ",\"device_index\":0,\"pipeline_operator_calls\":4,\"timing_method\":\"host_batch_submit_sync\""
         ...
         << ",\"normalization\":\"unnormalized inverse then one 1/M Mul\",\"scale_binding\":"
         << quote(vector_scale ? "full vector" : "complex64 scalar broadcast [1]")
